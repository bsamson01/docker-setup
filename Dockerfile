FROM ubuntu:latest
RUN mkdir /home/app
RUN apt-get update && apt-get install -y vim g++ gcc build-essential libssl-dev zsh wget git curl \
...
# RUN curl https://raw.githubusercontent.com/creationix/nvm/v0.20.0/install.sh | bash && . $NVM_DIR/nvm.sh && nvm install 13.6.0
...
RUN tar -xvzf ./jdk-13.0.1_linux-x64_bin.tar.gz -C /usr/lib/jvm
